...
pub fn remove_liquidity_amounts(
    ...
    events.iter().find_map(|event| match event {
        DecodedEvent::RemoveLiquidity(liq) if liq.position == *position => {
            Some(RecoveredAmounts {
...
/// `Swap` event (see `claim_fee_amounts`'s doc comment for why `ClaimFee` is different), so
/// nothing calls this outside its own test yet -- kept so that if a future caller ever needs a
/// swap's LP-earned amount out of a decoded transaction, it reaches for the field this
/// codebase's own established rule says is correct, instead of re-deriving the subtraction or
...
mod tests {
    use super::*;
    use dlmm_decode::{DecodedClaimFee, DecodedLiquidityEvent, DecodedSwap, LiquidityEventKind};
    use solana_sdk::pubkey::Pubkey;
    ...
    // (`event.fee` includes the protocol's cut; LPs earn `fee - protocol_fee`). This is the
    // case that would pass if `swap_lp_fee` returned the raw `fee` field instead of
    // `lp_fee`: fee=1_000, protocol_fee=100 would make a caller that used `fee` directly
    // overstate the LP's earnings by exactly the protocol's cut.
    #[test]
