-- Hierarchical over pool_metrics_4h, same pattern as 0013.
CREATE MATERIALIZED VIEW pool_metrics_24h
WITH (timescaledb.continuous) AS
SELECT
    pool_address,
    time_bucket('24 hours', bucket_start) AS bucket_start,
...
GROUP BY pool_address, time_bucket('24 hours', bucket_start)
WITH NO DATA;

SELECT add_continuous_aggregate_policy('pool_metrics_24h',
    start_offset => INTERVAL '3 days',
    end_offset => INTERVAL '4 hours',
    schedule_interval => INTERVAL '4 hours'
);

CREATE INDEX idx_pool_metrics_24h_bucket ON pool_metrics_24h (bucket_start DESC, pool_address);
